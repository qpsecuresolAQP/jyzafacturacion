<?php $this->assign('title', 'Reporte de Usuarios y Procedencia'); ?>

<?= $this->Form->create(null, ['type' => 'get', 'class' => 'mb-3']) ?>
<fieldset>
    <legend>Filtrar por Rango de Fechas, Usuario y Departamento</legend>
    <div class="row">
        <div class="col">
        ...
        <div class="col">
            <?= $this->Form->control('departamento_id', [
                ...
                'empty' => 'Seleccione lugar de procedencia',
                'label' => 'Procedencia',
                'class' => 'form-control',
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        document.querySelector('form').addEventListener('submit', function (event) {
            ...
            const departamentoId = document.querySelector('select[name="departamento_id"]').value;
            ...
            const url = `<?= $this->Url->build(['controller' => 'VistaReportePacientes', 'action' => 'exportarPdf']) ?>?usuario_id=${usuarioId}&departamento_id=${departamentoId}&start_date=${startDate}&end_date=${endDate}`;
        ...
        document.getElementById('exportPdfBtn').addEventListener('click', function () {
            ...
            const url = `<?= $this->Url->build(['controller' => 'VistaReportePacientes', 'action' => 'exportarPdf']) ?>?usuario_id=${usuarioId}&departamento_id=${departamentoId}&start_date=${startDate}&end_date=${endDate}&download=1`;
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        document.getElementById('exportExcelBtn').addEventListener('click', function (event) {
            ...
            const url = `<?= $this->Url->build(['controller' => 'VistaReportePacientes', 'action' => 'exportarExcel']) ?>?${params.toString()}`;
            window.location.href = url;
        });
    });    
</script>
